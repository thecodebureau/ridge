...
var View = Backbone.View.extend({
	...
	constructor: function(options) {
		_.extend(this, _.pick(options, 'template', 'data', 'parent'));
	...
	render: function() {
		...
		if (this.model) {
			if(args){ 
				var model = {};
				model[this.model.name.toCamelCase()] = this.model.toJSON();
			}

			data = _.defaults(model || this.model.toJSON(), data);
		} else if(this.collection)
			data = _.defaults(this.collection.toJSON(), data);
		
		this.renderTemplate(data).ready(args || arguments);
