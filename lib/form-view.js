...
module.exports = Bassline.View.extend({
	...
	initialize: function () {
		...
		// set the form view up to listen to edit request for
		// it's model.
		this.listenTo(app, 'edit:' + this.model.name, this.setModel);
	},
	...
	attach: function() {
		...
		if(!this.controls) {
			...
			this.controls = new app.views.FormControls({ el: this.$('.controls'), model: this.model });
		} else {
		...
		this.$('.image-upload').each(function() {
			...
			new app.views.ImageUpload({ el: this, model: view.model });
		});
	...
	attachProperties: function() {
		...
		}).not('.spytext-field').on('keyup', function() {
