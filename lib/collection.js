module.exports = Backbone.Collection.extend({
	constructor: function(attributes, options) {
		...
		_collection.defaultFilter = _collection.filter = options && options.defaultFilter ? options.defaultFilter : { 
			_sort: '-dateCreated',
			_limit: 10
		};
	...
	fetch: function(options) {
		...
		//	_collection.totalCount = options.xhr.getResponseHeader('X-Collection-Length');
	...
	setFilter: function(filter) {
		if(filter)
			this.filter = _.extend({}, this.defaultFilter, filter);
		else
