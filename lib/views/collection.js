module.exports = {
	...
	attach: function() {
		...
		var $models = _view.$el.children('.collection');

		_view.elements.$models = $models.length > 0 ? $models : _view.$el;
	...
	add: function(model, reset) {
		...
		if(!this.ModelView.prototype.template)
			obj.template = this.templateRoot + 'partials/' + this.collection.modelName.toSpinalCase();
